...
def get_pixel_coordinates(mask):
    ...
    y, x = np.where(mask)
    pts = np.concatenate(
        (x[:, np.newaxis], y[:, np.newaxis], np.ones((x.size, 1))), axis=1
    )  # rows of [x1, y1, 1]
    print(f"Homogeneous coordinates array (x, y, 1):\n {pts}")
...
def fit_image_in_target_space(img_src, img_dst, mask, H, offset=np.array([0, 0, 0])):
    ...
    pts = get_pixel_coordinates(mask)  # rows of [x1, y1, 1]
    ...
    # Return only x, y non-homogeneous coordinates
    out_src = out_src[0:2, :]
    out_src = out_src.T
...
def get_pixel_value(img_dst, img_src, pts, out_src, offset):
    """
    :param img_dst:
    ...
    """
    h, w, _ = img_src.shape
    tl = np.floor(out_src[:, ::-1]).astype(
        np.int64
    )  # reverse cols to get row, col notation
...
def build_system_equations(in_pts, out_pts):
    """
...
def run(img_src_path, img_dst_path, out_pts, include_perp=False, save_fig="result.jpg"):
    """
...
def plot_req_images(img_src, img_dst, mask, fig_name):
    plt.figure()
