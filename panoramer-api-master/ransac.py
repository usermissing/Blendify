...
class RANSAC:
    ...
    def get_inliers(self, H, pts_in_expected, delta):
        ...
        pts_in = convert_to_homogeneous_coordinates(
            pts_in, axis=1
        )  # rows of [x1, y1, 1]
    ...
    def run_ransac(self, correspondence):
        ...
        print(
            "N: {}, n: {}, M:{}, p: {}, eps: {}, delta: {}".format(
                self.N, self.n, self.M, self.p, self.eps, self.delta
            )
        )
        no_iter = 0
        ...
        while no_iter <= self.N:
            ...
            H = calculate_homography(
                in_pts=sample_pts[:, 2:], out_pts=sample_pts[:, 0:2]
            )
        ...
        final_corr_points = np.concatenate(
            (current_sample_pts, current_inliers), axis=0
        )
        final_H = calculate_homography(
            in_pts=final_corr_points[:, 2:], out_pts=final_corr_points[:, 0:2]
        )
